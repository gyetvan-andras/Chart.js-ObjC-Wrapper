...
@property (nonatomic,strong) NSColor* scaleGridLineColor;// : "rgba(0,0,0,.05)",
...
@property (nonatomic,strong) NSNumber* legendTemplate;// : "<ul class=\"<%=name.toLowerCase()%>-legend\"><% for (var i=0; i<datasets.length; i++){%><li><span style=\"background-color:<%=datasets[i].strokeColor%>\"></span><%if(datasets[i].label){%><%=datasets[i].label%><%}%></li><%}%></ul>"
